  ...
  <div class="table-wrapper">
    <div class="table-title">
      <div class="row">
        ...
        <div class="col-sm-6">
          <a href="/new-post" class="btn btn-success d-flex justify-content-center align-items-center gap-1">
      ...
      <tbody id="list">
        @foreach($posts as $post)
          <tr>
            <td>{{ $post->id }}</td>
            <td>{{ $post->title }}</td>
            <td>{{ $post->truncatedContent() }}</td>
            <td>{{ $post->author->name }}</td>
            <td>
              <div class="d-flex">
                <a class="btn btn-sm btn-warning" href="/edit-user/{{ $post->id }}">
                ...
                <button class="ms-1 btn btn-sm btn-danger" onclick="deleteUser({{ json_encode($post) }})">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="#ffffff" class="w-5 h-5">
